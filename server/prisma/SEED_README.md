...
此文件包含 nest-admin-soybean 系统的初始化种子数据，用于在开发和测试环境中快速搭建基础数据。

## SQL 转换脚本

如果需要从 `init.sql` 重新生成种子数据，可以使用转换脚本：

```bash
# 运行转换脚本
npx ts-node prisma/convert-sql-to-seed.ts
```

转换脚本会：
1. 读取 `db/init.sql` 文件
2. 解析所有 INSERT 语句
3. 转换为 Prisma seed 格式
4. 生成 `prisma/seed.ts` 文件
